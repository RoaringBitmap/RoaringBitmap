...
public class Roaring64NavigableMap implements Externalizable, LongBitmapDataProvider {
    ...
    public void serialize(ByteBuffer out) throws IOException {
        out.order(BIG_ENDIAN);
        out.put(signedLongs ? (byte) 1 : (byte) 0);

        out.putInt(highToBitmap.size());
    ...
    public void deserialize(ByteBuffer bf) throws IOException {
        this.clear();

        ByteBuffer buffer = bf.slice();
        buffer.order(BIG_ENDIAN);

        boolean signedLongs = buffer.get() == 0x00 ? false : true;
        int nbHighs = buffer.getInt();
        ...
        for (int i = 0; i < nbHighs; i++) {
            ...
            buffer.position(buffer.position() + provider.serializedSizeInBytes());
            highToBitmap.put(high, provider);
        }
