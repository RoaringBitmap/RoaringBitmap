...
public class TestUtil {
    ...
    public void testCopy() {
      ...
      for(int k = 16; k < 32; ++k)
        Assert.assertEquals((char)k,dest.get(k));
      BufferUtil.arraycopy(slice, 0, dest, 16, 16);
      for(int k = 16; k < 32; ++k)
        Assert.assertEquals((char)(k-16),dest.get(k));
  ...
  @Test
    public void testAdvanceUntil() {
        ...
        Assert.assertEquals(1, BufferUtil.advanceUntil(data, -1, data.limit(), (char) 3));
        Assert.assertEquals(5, BufferUtil.advanceUntil(data, -1, data.limit(), (char) 28));
        Assert.assertEquals(5, BufferUtil.advanceUntil(data, -1, data.limit(), (char) 29));
        Assert.assertEquals(7, BufferUtil.advanceUntil(data, -1, data.limit(), (char) -342));
    }
