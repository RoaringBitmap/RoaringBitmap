...
public class RunArrayOrBenchmark {
    ...
    public int OrRunContainer(BenchmarkState benchmarkState) {
        ...
        for(int k = 1; k < benchmarkState.rc.size(); ++k)
            answer += RoaringBitmap.or(benchmarkState.rc.get(k-1),benchmarkState.rc.get(k)).getCardinality();
        return answer;
    }
    ...
    public int OrBitmapContainer(BenchmarkState benchmarkState) {
        ...
        for(int k = 1; k < benchmarkState.ac.size(); ++k)
            answer += RoaringBitmap.or(benchmarkState.ac.get(k-1),benchmarkState.ac.get(k)).getCardinality();
        return answer;
    }
