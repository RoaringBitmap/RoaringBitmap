...
public class RunArrayAndBenchmark {
    ...
    public int AndRunContainer(BenchmarkState benchmarkState) {
        ...
        for(int k = 1; k < benchmarkState.rc.size(); ++k)
            answer += RoaringBitmap.and(benchmarkState.rc.get(k-1),benchmarkState.rc.get(k)).getCardinality();
        return answer;
    }
    ...
    public int AndBitmapContainer(BenchmarkState benchmarkState) {
        ...
        for(int k = 1; k < benchmarkState.ac.size(); ++k)
            answer += RoaringBitmap.and(benchmarkState.ac.get(k-1),benchmarkState.ac.get(k)).getCardinality();
        return answer;
    }
