...
public class BasicAndNotContainerBenchmark {
	...
	public int andNotBitmapContainerVSRunContainerContainer(
		...
		if (benchmarkState.rc2.serializedSizeInBytes() > benchmarkState.ac2
			...
			throw new RuntimeException(
					"Can't expect run containers to win if they are larger.");
		return benchmarkState.ac1.andNot(benchmarkState.rc2).getCardinality();
	}

	@Benchmark
	public int andNotBitmapContainerVSBitmapContainer(
			BenchmarkState benchmarkState) {
		return benchmarkState.ac1.andNot(benchmarkState.ac2).getCardinality();
	}

	@Benchmark
	public int reverseAndNotBitmapContainerVSRunContainerContainer(
			BenchmarkState benchmarkState) {
		if (benchmarkState.rc2.serializedSizeInBytes() > benchmarkState.ac2
				.serializedSizeInBytes())
			throw new RuntimeException(
	...
	@Benchmark
	public int reverseAndNotBitmapContainerVSBitmapContainer(
			BenchmarkState benchmarkState) {
		return benchmarkState.ac2.andNot(benchmarkState.ac1).getCardinality();
	}

	@Benchmark
	public int part2_andNotRunContainerVSRunContainerContainer(
			BenchmarkState benchmarkState) {
		if (benchmarkState.rc2.serializedSizeInBytes() > benchmarkState.ac2
				.serializedSizeInBytes())
			throw new RuntimeException(
					"Can't expect run containers to win if they are larger.");
		if (benchmarkState.rc3.serializedSizeInBytes() > benchmarkState.ac3
				.serializedSizeInBytes())
			throw new RuntimeException(
					"Can't expect run containers to win if they are larger.");
	...
	@Benchmark
	public int part2_andNotBitmapContainerVSBitmapContainer2(
			BenchmarkState benchmarkState) {
		return benchmarkState.ac3.andNot(benchmarkState.ac2).getCardinality();
	}

	@Benchmark
	public int part2_reverseAndNotRunContainerVSRunContainerContainer(
			BenchmarkState benchmarkState) {
		if (benchmarkState.rc2.serializedSizeInBytes() > benchmarkState.ac2
				.serializedSizeInBytes())
			throw new RuntimeException(
	...
	@Benchmark
	public int part2_reverseAndNotBitmapContainerVSBitmapContainer2(
			BenchmarkState benchmarkState) {
	...
	@Benchmark
	public int part3_reverseAndNotArrayContainerVSRunContainerContainer(BenchmarkState benchmarkState) {
		if(benchmarkState.rc2.serializedSizeInBytes() > benchmarkState.ac2.serializedSizeInBytes())
			throw new RuntimeException("Can't expect run containers to win if they are larger.");
	...
	public static class BenchmarkState {
		...
		public BenchmarkState() {
			...
			int[] values4 = RandomUtil.generateUniformHash(rand, bitsetperword3
					* howmanywords, max);
