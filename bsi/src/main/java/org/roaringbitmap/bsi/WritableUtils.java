 ...
 * WritableUtils
 *
 * <p>copy from org.apache.hadoop.io.WritableUtils
 */
public class WritableUtils {
    /**
     * Serializes an integer to a binary stream with zero-compressed encoding. For -120 &lt;= i
     * &lt;= 127, only one byte is used with the actual value. For other values of i, the first byte
     * value indicates whether the integer is positive or negative, and the number of bytes that
     * follow. If the first byte value v is between -121 and -124, the following integer is
     * positive, with number of bytes that follow are -(v+120). If the first byte value v is between
     * -125 and -128, the following integer is negative, with number of bytes that follow are
     * -(v+124). Bytes are stored in the high-non-zero-byte-first order.
     *
