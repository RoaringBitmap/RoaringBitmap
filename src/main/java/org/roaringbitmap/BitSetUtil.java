...
public class BitSetUtil {
    ...
    /**
     * Compares a RoaringBitmap and a BitSet. They are equal
     * if and only if they contain the same set of integers.
     * 
    ...
    private static ArrayContainer arrayContainerOf(final int from,
        ...
        for (int i = from, socket = 0; i < to; ++i,socket += Long.SIZE) {
            ...
            while (word != 0) {
                long t = word & -word;
                content[index++] = (short) (socket + Long
                        .bitCount(t - 1));
                word ^= t;
            }
        }
    ...
    private static int cardinality(final int from, final int to,
            final long[] words) {
        int sum = 0;
